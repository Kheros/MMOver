...
$actiontemplates = array(
	'blog'				=>	array(
		...
		'ad_blog_showentry_after',
		'ad_blog_showentry_before',
		'ad_blog_showentry_firstcomment',
...
if ($_REQUEST['do'] == 'blog')
{
	...
	while ($comment = $db->fetch_array($comments))
	{
		...
		if ($comment['firstshown'])
		{
			$responsebits .= vB_Template::create('ad_blog_showentry_firstcomment')->render();
		}
...
if ($_REQUEST['do'] == 'list')
{
	...
	if (!empty($blogids))
	{
		...
		while ($blog = $db->fetch_array($blogs))
		{
			...
			{
				$blogbits .= vB_Template::create('ad_blog_list_firstentry')->render();
			}
