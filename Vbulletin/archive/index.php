...
{
	vbsetcookie('pda', '1', 1);
	exec_header_redirect('index.php/' . $querystring);
}
...
if ($do == 'forum')
{
	...
	$output .= "<p class=\"largefont\">$vbphrase[view_full_version] : <a href=\"" . $vbulletin->options['bburl'] . '/' . fetch_seo_url('forum', $foruminfo) . "\">$foruminfo[title_clean]</a></p>\n<hr />\n";
	...
	if ($foruminfo['cancontainthreads'])
	{
		...
		while ($thread = $db->fetch_array($threads))
		{
			...
			{
				$output .= "\t<li>$thread[prefix_plain_html] <a href=\"" . $vbulletin->options['bburl'] . '/' . fetch_seo_url('thread', $thread) . "\">$thread[title]</a></li>\n";
			}
		}
		$output .= "</ol>\n</div>\n";
...
if ($do == 'thread')
{
	...
	$output .= "<p class=\"largefont\">$vbphrase[view_full_version] : "
		...
		. "<a href=\"" . $vbulletin->options['bburl'] . '/' . fetch_seo_url('thread', $threadinfo) . "\">$threadinfo[title]</a></p>\n<hr />\n";
