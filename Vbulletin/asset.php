...
if (empty($_REQUEST['fid']) OR empty($_REQUEST['uid']))
{
...
if (!empty($_SERVER['HTTP_IF_MODIFIED_SINCE']) OR !empty($_SERVER['HTTP_IF_NONE_MATCH']))
{
	...
	{
		header('ETag: "' . intval($_REQUEST['fid']) . '-' . intval($_REQUEST['uid']) . '"');
	}
	exit;
}
...
if (
	...
	!$vbulletin->GPC['fid']
		OR
	(
		$vbulletin->GPC['uid'] != $vbulletin->userinfo['userid']
			AND
		!can_moderate()	// this is leniant for performance sake, may need to change
	)
		OR
