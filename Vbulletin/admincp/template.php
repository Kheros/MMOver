...
define('CVS_REVISION', '$RCSfile$ - $Revision: 35700 $');
...
if ($_REQUEST['do'] == 'download')
{
	...
	else
	{
		...
		if ($vbulletin->GPC['mode'] == 2)
		{
			...
			$is_master = true;
			$vbulletin->GPC['title'] = $vbphrase['master_style'];
		}

		//export with parent styles
		else if ($vbulletin->GPC['mode'] == 1)
		{
	...
	while ($gettemplate = $db->fetch_array($gettemplates))
	{
		switch($gettemplate['templatetype'])
		{
			case 'template': // regular template
				...
				if (substr($gettemplate['title'], 0, 3) == 'ad_' AND $vbulletin->GPC['mode'] == 2)
				{
	...
	$xml->add_group('style',
		array(
			...
			'vbversion' => $full_product_info[$vbulletin->GPC['product']]['version'],
