...
/*======================================================================*\
|| #################################################################### ||
|| # vBulletin 4.0.3 - Licence Number VBF1Y7SPM3
|| # ---------------------------------------------------------------- # ||
...
if ($_REQUEST['do'] == 'fbdisconnect')
{
	...
	if (is_facebookenabled() AND !empty($vbulletin->userinfo['userid']) AND $vbulletin->userinfo['logintype'] == 'vb')
	{
		...
		if ($vbulletin->GPC['confirm'])
		{
			...
			// go to facebook site to logout, then redirect back to main forum page
...
// ############################### start facebook auto-register ###############################
// process facebook association
if ($_POST['do'] == 'autoregister')
{
	// make sure all the conditions are right for auto-register,
	// if not, just direct them to the regular register form
	if (!is_facebookenabled()
		OR !vB_Facebook::instance()->userIsLoggedIn()
		OR !$vbulletin->options['facebookautoregister'])
	{
		$_REQUEST['do'] = 'register';
	}
	else 
...
{	
	...
	// if facebook connect auto register is enabled, and the user is not logged in to vB
	// but is logged into FB, add the facebook auto register client scripts to the page
	if (is_facebookenabled()
		AND empty($vbulletin->userinfo['userid']) /* not logged into vb */
		AND	vB_Facebook::instance()->userIsLoggedIn() /* logged into facebook */
		AND (empty($userdata) OR empty($userdata->errors))/* make sure we are not trying to display errors */
		AND $vbulletin->options['facebookautoregister'])
	{
		$show['fb_autoregistration'] = true;
	}
	else
	{
		$show['fb_autoregistration'] = false;
	}
